...
function CampaignFormModal({ campaign, onSave, onClose }) {
    const initialValues = { titulo: '', descripcion: '', fecha_inicio: new Date().toISOString().split('T')[0], fecha_fin: '', monto_objetivo: 0, permite_donacion_monetaria: true, permite_donacion_especie: true, categoria: [], tipo_impacto: [], imagen_url: '', objetivos: [], galeria_imagenes: [], video_urls: [], necesidades: [] };
            ...
            <div className="relative bg-surface rounded-3xl shadow-elevation-5 w-[90vw] max-w-2xl max-h-[90vh] flex flex-col overflow-hidden animate-slide-up">
                ...
                <div className="flex-1 overflow-y-auto p-6 scroll-smooth min-h-[50vh]">
                    <form id="campaign-form" onSubmit={handleSubmit} className="space-y-6">
                        ...
                        <DynamicList label="Galería de Imágenes" items={formData.galeria_imagenes} onAdd={(v)=>setFormData(p=>({...p, galeria_imagenes: [...p.galeria_imagenes, v]}))} onRemove={(i)=>setFormData(p=>({...p, galeria_imagenes: p.galeria_imagenes.filter((_,x)=>x!==i)}))} placeholder="https://..." type="url" maxItems={6} />
                        <DynamicList label="Videos (YouTube, Instagram, TikTok)" items={formData.video_urls} onAdd={(v)=>setFormData(p=>({...p, video_urls: [...p.video_urls, v]}))} onRemove={(i)=>setFormData(p=>({...p, video_urls: p.video_urls.filter((_,x)=>x!==i)}))} placeholder="https://..." type="url" maxItems={5} />
                    </form>
                </div>
